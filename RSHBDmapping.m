% Raman Spectrum Hydrogen Bond Deconvolution (HBDRS)
...
SizeMyData=size(RamanData);
SampleNum=SizeMyData(2)/2;
HB_Parameter=zeros(SampleNum,10);
HBTI=zeros(SampleNum,1);

for Series = 1:1:SampleNum
    RamanshiftOri = table2array(RamanData(:,Series*2-1)); % x axis of Raman Intensity % change Wavenumber formate to double
    IntensityOri = table2array(RamanData(:,Series*2)); % y axis of Raman Intensity % change Intensity formate to double
    ...
    HB_Parameter(Series,:)=parameter;
    HBTI(Series,1)=(parameter(6)+parameter(8)+parameter(10))/(parameter(2)+parameter(4));
Series % mark the current sequence number
toc    % mark the current sequence time
end

xlsName = strcat(datestr(datetime),'.xlsx');
xlsName = strrep(xlsName,':','');
HB_Parameter_title = ["PeakDAA","AreaDAA","PeakDDAA","AreaDDAA","PeakDA","AreaDA","PeakDDA","AreaDDA","PeakDDAA","AreaDDAA","PeakFREE","AreaFREE"];
xlswrite(xlsName,HB_Parameter_title,'HB_Parameter','A1:J1'); 
xlswrite(xlsName,HB_Parameter,'HB_Parameter','A2'); 
xlswrite(xlsName,HBTI,'HBTI'); 
